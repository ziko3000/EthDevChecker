...
export class HelpCommand extends Command {
  name = 'help';
  description = 'About bot and its commands, support';
  ...
  async execute(interaction: CommandInteraction): Promise<void> {
    try {
      const helpEmbed = new EmbedBuilder()
        .setTitle('Bot Help')
        .setDescription('This bot provides information about the latest big Ethereum Foundation transaction. When there is a new big transaction, the message automatically is being sent to the channel.')
        .setColor('#0099ff')
        .addFields(
